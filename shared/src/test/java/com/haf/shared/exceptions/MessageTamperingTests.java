...
class MessageTamperingTests {
    ...
    @Test
    void test_tampered_IV_fails() throws Exception {
        ...
        char last = m.getIvB64().charAt(m.getIvB64().length() - 1);
        char flip = (last == 'A') ? 'B' : 'A';
        m.setIvB64(m.getIvB64().substring(0, m.getIvB64().length() - 1) + flip); // flip last char of IV
    ...
    @Test
    void test_wrong_aad_fails() throws Exception {
        KeyPair kp = EccKeyIO.generate();
        ...
        m.setRecipientId("userC"); // tamper AAD field used in AadCodec
        assertThrows(MessageTamperedException.class, () -> dec.decryptMessage(m), "Tampered AAD must fail decrypt");
    }

    @Test
    void test_tampered_ciphertext_fails() throws Exception {
        KeyPair kp = EccKeyIO.generate();
        ...
        char last = m.getCiphertextB64().charAt(m.getCiphertextB64().length() - 1);
        char flip = (last == 'A') ? 'B' : 'A';
        m.setCiphertextB64(m.getCiphertextB64().substring(0, m.getCiphertextB64().length() - 1) + flip); // flip last
                                                                                                         // char
    ...
    @Test
    void test_tampered_tag_fails() throws Exception {
        KeyPair kp = EccKeyIO.generate();
        ClockProvider clock = new FixedClockProvider(1000000L);
        ...
        char last = m.getTagB64().charAt(m.getTagB64().length() - 1);
        char flip = (last == 'A') ? 'B' : 'A';
        m.setTagB64(m.getTagB64().substring(0, m.getTagB64().length() - 1) + flip);
