...
- Starts ingress servers:
    - 'HttpIngressServer' in 'HAF_HTTP_PORT' for '/api/v1/messages' (POST).
    - 'WebSocketIngressServer' in 'HAF_WS_PORT' for '/ws' (persistent bidirectional connections).
- Registers graceful shutdown hook:
    - In SIGTERM/SIGINT, it stops the servers with a timeout.
...
- Configuration errors:
    - If 'ServerConfig' throws exception (missing env var, invalid format), 'Main' log fatal error and makes 'System.exit(1)'.
- Migration errors:
    - If Flyway fails, log the stack trace and terminates with a non-zero exit code.
- TLS errors:
    - If the keystore path does not exist or the code is incorrect, it throws exception and abort.
- Runtime errors:
