...
- `SearchViewModel` executes async authenticated requests to `GET [REST_PATH]/search?q=<query>&limit=<n>[&cursor=<token>]`.
...
4. ViewModel sends authenticated request to the search endpoint (default `/api/v1/search`) with configured page size and optional cursor.
