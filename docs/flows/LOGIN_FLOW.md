...
2. **Token Hydration**: Server returns an access JWT in `sessionId` plus a refresh token. The access JWT is used as `Authorization: Bearer ...` for REST/WebSocket authenticated calls.
3. **Transport Hydration**: It sets up `WebSocketAdapter` with runtime-resolved URIs and the access JWT emitted by the server HTTP response.
...
5. **Messenger Tying**: It bundles the crypto-provider and adapter into `DefaultMessageSender` and `DefaultMessageReceiver`, with receiver transport mode chosen by `ClientRuntimeConfig` (`WEBSOCKET` in dev, `HTTPS_POLLING` in prod).
